...
fun ResponseBody.readLinesOfHtml(): List<String> {
    ...
    val charset = inputStream0.bufferedReader().readLines().firstOrNull { it.toLowerCase(Locale.ENGLISH).contains("charset=\"?[^\"^\n^;]+\"?".toRegex()) }?.let {
        StringKtUtil.getDataFromContentByRegex(it.toLowerCase(Locale.ENGLISH), "charset=\"?([^\"^\n^;]+)\"?", listOf(1))?.get(0)
